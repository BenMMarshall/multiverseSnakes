...
```{r hypothesesFigure, echo=FALSE, out.width='100%', fig.height=5, fig.width=3, fig.align="centre", fig.cap="..."}
...
To reacquaint ourselves with the data, and check for any potential issues that could hinder habitat selection analysis, we used code from @crane_lots_2021 to generate summaries of all the tracking datasets.
...
```{r decisionsFigure, echo=FALSE, out.width='100%', fig.height=5, fig.width=3, fig.align="centre", fig.cap="..."}
...
However, the impacts of the inclusion have differing impacts when using step-selection or a Poisson model <!--- chapter 3 results-->; we include it here to explore how dramatic that difference can be.
...
The rarity of these habitats may have also played a role, as we see clear decrease in the confidence intervals surrounding the estimates as we increased the number of available points sampled <!--- cite supp figures here -->.
...
The weighted Resource Selection Functions should be best placed to provide us with a broad estimate of habitat selection, while also best positioned to tackle the issues arising from inconsistent tracking [@alston_mitigating_2023].
...
The previously published conclusions that they make heavy use of semi-natural areas (i.e., banks of irrigation canals, and scrub between fields) is nearly universally supported regardless of approach, especially when classified as invert continuous distance.
Why *Ophiophagus hannah* uses these habitats is not immediately clear, but it could be driven by prey availability, the avoidance of threats, and density of shelter sites.
Other animals are know to make use of linear habitats <!-- cites from orig paper -->, including other snakes at this study site (i.e., *Ophiophagus hannah* primary prey).
...
Human activity in fields, such as ploughing will kill snakes <!-- krait mort paper here -->, and we have documented many fatalities of *Ophiophagus hannah* as a direct result of human actions (e.g., persecution, road collision) <!-- king mort paper -->.
...
The original paper highlighted the propensity for *Ophiophagus hannah* to leave the forest <!-- cite -->, and the heightened movement particularly during breeding season, potentially mate searching behaviour, was suggested as a likely driver <!-- cite -->.
...
However, data collected after the original paper showed the use of forest for nesting sites for some *Ophiophagus hannah*, and their use of semi-natural areas to access it.
This highlights the potential importance of forest during certain times of year, even if it is less likely to be identified by the broad (rather generalised) habitat selection multiverse-analysis here.
...
Whereas for food, rice paddies can house suitable sized water birds [@fujioka_bird_2010], and *Python bivitattus* in this area have been documented depredating domestic waterfowl <!-- smith paper cite -->, together supporting suggestions that the water bodies are a resource.
...
However, the use of exact selection strengths should be examined in more detail if they are to be adapted to future purposes such as human-snake conflict prediction (as supported by simulation habitat multiveres; @marshall_habitat_2024; Chapter 3).
